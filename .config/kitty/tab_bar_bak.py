...
ICON_FG: int = as_rgb(color_as_int(opts.color3))
ICON_BG: int = as_rgb(color_as_int(opts.color0))

CLOCK_FG = as_rgb(color_as_int(opts.color3))
CLOCK_BG = as_rgb(color_as_int(opts.color0))
DATE_FG = as_rgb(color_as_int(opts.color3))
DATE_BG =  as_rgb(color_as_int(opts.color0))
...
def _draw_right_status(screen: Screen, is_last: bool) -> int:
    ...
    cells = [
        # (CLOCK_FG, CLOCK_BG, datetime.datetime.now().strftime("􀺊  %I:%M %p ")),
        # (DATE_FG, DATE_BG, datetime.datetime.now().strftime("󰃰 %A ").upper()),
        (DATE_FG, DATE_BG, datetime.datetime.now().strftime("􀉉  %a-%d %I:%M%p ").upper()),
    ]
