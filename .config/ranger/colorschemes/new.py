...
from ranger.gui.color import black, white, green, red, blue, magenta, yellow, cyan, bold, underline, dim, reverse, normal, default, BRIGHT
...
class ColorStack:
    ...
    def copy(self, other):
        ...
        self._set(fg=other._fg, bg=other._bg, bright=other._bright, bold=other._bold, underline=other._underline, dim=other._dim, reverse=other._reverse)
        return self

    def add(self, other):
        """Override explicitly set attributes, which is the current 'inheritance' approach."""
        self._add(fg=other._fg, bg=other._bg, bright=other._bright, bold=other._bold, underline=other._underline, dim=other._dim, reverse=other._reverse)
        return self
...
class Custom(ColorScheme):
    ...
    BROWSER_COLORS = {
        'selected': ColorStack(is_bright=True, is_bold=True, is_dim=False, is_reverse=True),  # reverse, fixed colors, underline, etc
        ...
        'tag_marker': ColorStack(fg=red, is_bright=True, is_bold=True),  # asterisk
        'tagged': ColorStack(),
        'infostring': ColorStack(),  # size
        'inactive_pane': ColorStack(fg=cyan, bg=normal),  # multi-pane multi-tab
    }

    VCS_COLORS = {
        'status': ColorStack(is_bright=False, is_bold=False, is_underline=False),  # default
    ...
    STATUS_COLORS = {
        ...
        'marked': ColorStack(fg=yellow, is_bright=True, is_bold=True, is_reverse=True),  # Mrk
    ...
    def use(self, context):
        ...
        if context.in_browser:
            if context.empty or context.error or context.badinfo:
                ...
                colors.copy(self.LS_COLORS['link'] if context.good else self.LS_COLORS['link_bad'])
            elif context.fifo:
                ...
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['executable'])
            elif context.container:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['container'])
            elif context.document:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['document'])
            elif context.video:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['video'])
            elif context.audio:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['audio'])
            elif context.image:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['image'])
            elif context.media:
                colors.copy(self.LS_COLORS['file']).add(self.LS_COLORS['media'])
            elif context.file:
            ...
            if context.hostname:
                colors.add(self.TITLE_COLORS['hostname_bad'] if context.bad else self.TITLE_COLORS['hostname'])
            elif context.directory:
                ...
                colors.add(self.TITLE_COLORS['tab_good'] if context.good else self.TITLE_COLORS['tab'])

        elif context.in_statusbar:
            colors.copy(self.STATUS_COLORS['in_statusbar'])

            if context.permissions:
                colors.add(self.STATUS_COLORS['permissions'] if context.good else self.STATUS_COLORS['permissions_bad'])
            elif context.nlink:
                ...
                colors.add(self.STATUS_COLORS['message_bad'] if context.bad else self.STATUS_COLORS['message'])
            ...
            if context.text:
                colors.add(self.STATUS_COLORS['text_highlight'] if context.highlight else self.STATUS_COLORS['text'])
        ...
        if context.vcsfile and not context.selected:
            if context.vcsconflict:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['conflict'])
            elif context.vcsuntracked:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['untracked'])
            elif context.vcschanged:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['changed'])
            elif context.vcsunknown:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['unknown'])
            elif context.vcsstaged:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['staged'])
            elif context.vcssync:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['sync'])
            elif context.vcsignored:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['ignored'])
        elif context.vcsremote and not context.selected:
            if context.vcssync or context.vcsnone:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['sync'])
            if context.vcsnone:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['none'])
            elif context.vcsbehind:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['behind'])
            elif context.vcsahead:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['ahead'])
            elif context.vcsdiverged:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['diverged'])
            elif context.vcsunknown:
                colors.add(self.VCS_COLORS['status']).add(self.VCS_COLORS['unknown'])
